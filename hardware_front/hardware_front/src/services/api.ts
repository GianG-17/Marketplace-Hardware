...
async function req<T>(path: string, options?: RequestInit): Promise<T> {
  ...
  if (!res.ok) {
    const error = new Error(`Erro ${res.status}`) as Error & { status?: number }
...
export type Avaliacao    = { id: number; cliente_id: number; produto_id: number; proposta_id: number; nota: number; comentario?: string; criado_em?: string; cliente?: { nome: string }; produto?: { nome_modelo: string; marca: string; imagens: string } }
